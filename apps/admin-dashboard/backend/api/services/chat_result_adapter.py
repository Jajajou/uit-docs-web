...
def _reference_candidates(result: dict[str, Any], response_text: str) -> list[Any]:
    for key in ("references", "sources", "retrieved_chunks", "source_documents", "documents", "chunks"):
        raw_references = result.get(key)
        if isinstance(raw_references, list) and raw_references:
            return raw_references
    return _extract_markdown_references(response_text)


def _normalize_reference_item(item: dict[str, Any], index: int) -> dict[str, Any]:
    reference = dict(item)
    metadata = item.get("metadata")
    normalized_metadata = metadata if isinstance(metadata, dict) else {}

    reference.setdefault("reference_id", f"ref-{index}")
    for key in ("title", "document_title", "file_name", "filename"):
        value = str(reference.get(key) or normalized_metadata.get(key) or "").strip()
        if value:
            reference["title"] = value
            break

    for key in ("excerpt", "chunk", "content", "page_content", "text"):
        value = str(reference.get(key) or normalized_metadata.get(key) or "").strip()
        if value:
            reference["excerpt"] = value
            break
...
def _normalize_references(result: dict[str, Any], response_text: str) -> list[dict[str, Any]]:
    raw_references = _reference_candidates(result, response_text)
    ...
    for index, item in enumerate(raw_references, start=1):
        if not isinstance(item, dict):
            continue
        normalized.append(_normalize_reference_item(item, index))
    return normalized
