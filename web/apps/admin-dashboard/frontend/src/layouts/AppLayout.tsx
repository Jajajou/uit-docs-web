...
function isDedicatedChatShellRoute(pathname: string) {
...
function isDocumentsRoute(pathname: string) {
    return pathname === '/documents' || pathname === '/library' || pathname.startsWith('/documents/')
}
...
export default function AppLayout() {
    ...
    const hasDedicatedChatShell = isDedicatedChatShellRoute(location.pathname)
    const fullBleedSurface = isFullBleedRoute(location.pathname)

    return (
        <div className="min-h-screen text-gray-900 transition-colors dark:text-white">
            {hasDedicatedChatShell ? null : (
                    ...
                    <div className="mx-auto flex max-w-[1760px] flex-wrap items-center gap-3 px-4 py-4 md:px-8">
                        <RouteIntentLink to="/chat" className="min-w-fit">
                            <div className="flex items-center gap-3">
                                <BrandMark className="h-14 w-14 rounded-[1.3rem]" label="UIT" />
                                <div className="flex flex-col">
                                    <div className="text-[0.65rem] font-semibold uppercase tracking-[0.32em] text-brand-500">UIT Portal</div>
                        ...
                        <nav className="order-3 flex w-full gap-2 overflow-x-auto pb-1 no-scrollbar md:order-2 md:w-auto md:flex-1 md:justify-center md:pb-0">
                            {availableNav.map((item) => {
                                const isActive =
                                    ...
                                    (item.path === '/documents' && isDocumentsRoute(location.pathname))
            ...
            <main className={cn('flex-1', hasDedicatedChatShell || fullBleedSurface ? 'px-0 py-0' : 'px-4 py-6 md:px-8 md:py-8')}>
                <AnimatePresence mode="wait">
                    <motion.div
                        ...
                        className={cn(
                            hasDedicatedChatShell ? 'min-h-screen' : fullBleedSurface ? 'min-h-[calc(100vh-5.5rem)]' : 'mx-auto max-w-7xl',
                        )}
                    >
                        <Outlet />
                    </motion.div>
