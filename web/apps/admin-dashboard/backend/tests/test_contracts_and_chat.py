...
def test_chat_stream_returns_502_when_live_lightrag_fails(client, monkeypatch):
    ...
    # When the live LightRAG backend fails, /api/chat/stream returns a graceful
    # fallback response (HTTP 200) with a `live_backend_unavailable` warning so
    # the UI can keep rendering rather than surfacing a hard 502 to end users.
    # The fallback is signalled via a low confidence score and the warning code.
    assert response.status_code == 200
    body = response.json()
    warnings = body["message"].get("warnings", [])
    warning_codes = {warning.get("code") for warning in warnings}
    assert "live_backend_unavailable" in warning_codes
    assert body["message"]["confidence"] <= 0.3
